...
**Data model** (`app/models.py`): collections `grid_substations`, `substations`, `transformers`, `feeders`, `users`, `audit_logs`. A substation owns transformers and feeders by `substation_id`. `feeder_type` is the key discriminator: `incoming_33kv` | `transformer_hv` | `incomer_11kv` | `outgoing_11kv` | `bus_coupler` | `lilo_33kv`.

**Topology is derived, never stored by hand.** `infer_topology(feeders)` computes `bus_config` (`single_bus` / `double_bus` / `double_bus_coupler` / `ring_main`) plus counts. Call `refresh_substation_topology()` (`app/routes/helpers.py`) after any feeder mutation so the stored `substation.topology` stays in sync. The SLD and PDF generators branch on `bus_config`.
...
- `sld_generator.py` (`SLDGenerator`) — builds an SVG string from raw `sym_*` element helpers; takes a `db`-like object (real `mongo.db` or a fake). Colours: 33kV `#CC2200`, 11kV `#0055CC`, bus `#111111`, earth `#006600`.
