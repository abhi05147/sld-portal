...
**Topology is derived, never stored by hand.** `infer_topology(feeders)` computes `bus_config` ∈ `{single_bus, sectionalized_11kv, sectionalized_33kv, sectionalized_both}` plus section/coupler counts (`num_11kv_sections`, `has_11kv_bus_coupler`, `has_33kv_bus_coupler`, `has_station_transformer`, `outgoing_33kv_count`). Keep the stored `substation.topology` in sync: `refresh_substation_topology()` (`app/routes/helpers.py`) runs after every feeder/transformer create/update/delete, and `ExcelImporter._import_sheet` refreshes it on (re)import. `sld_generator._layout` still derives the bus configuration it renders straight from the feeders passed to it (not the stored value); `pdf_generator` reads `substation.topology` for its summary table. `infer_topology` also treats the retired `lilo_33kv` feeder type as `outgoing_33kv` so pre-migration rows still count.
